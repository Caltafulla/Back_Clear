  ...
  return (
    <div style={{ padding: 24, display: 'grid', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
        ...
        <StatCard title="Users" value={data?.users?.total ?? 0} icon="👥" accent="var(--success)" />
        <StatCard title="Active Evaluations" value={data?.evaluations?.active ?? 0} icon="⏱️" accent="var(--warning)" />
        <StatCard title="Completed Evaluations" value={data?.evaluations?.completed ?? 0} icon="✅" accent="var(--gray-700)" />
      </div>
      <div>
        <h2 style={{ margin: '12px 0 8px' }}>Quick Links</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12, marginTop: 8 }}>
          <QuickLink href="/challenge-management" icon="⚙️" label="Manage Challenges" />
          <QuickLink href="/course-management" icon="🏫" label="Manage Courses" />
          <QuickLink href="/evaluation-management" icon="📝" label="Manage Evaluations" />
